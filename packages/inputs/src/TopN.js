...
import { Query, desc, isParamLike } from '@uwdata/mosaic-sql';
...
export class TopNSelector extends MosaicClient {
  ...
  constructor({
    ...
    if (isParamLike(this.value)) {
      this.value.addEventListener('value', () => {this.requestUpdate();});
    }
  ...
  query(filter = []) {
    const { from, by, otherBy, value, n } = this;
